...
export default function JobDetailPage() {
  ...
  const {
    ...
    ...(job && {
      values: {
        scheduledAt: job.scheduledAt.slice(0, 16), // datetime-local format
  ...
  if (!job) {
    return (
      <div className="p-8">
        ...
        <Link to="/owner/jobs" className="mt-2 text-sm text-blue-600 hover:underline">
          Back to jobs
        </Link>
      </div>
    )
  ...
  return (
    <div className="p-8">
      ...
      {/* Checklist progress (read-only on this page — crew fills it in on mobile) */}
            ...
            {job.checklistItems.map((item) => (
              <li key={item.id} className="flex items-center gap-3 py-3">
                ...
                {item.photos.length > 0 && (
                  <span className="text-xs text-green-600">{item.photos.length} photo(s)</span>
                )}
              </li>
            ))}
          </ul>
...
const inputClass =
  'w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 disabled:bg-gray-50 disabled:text-gray-400'
